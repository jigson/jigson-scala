  ...
  implicit class JIGToJSON (val v: JIGValue) {
    def toJSON(indent : Int = 0) : String = {
      ...
      v match {
        ...
        case JIGNull() => "null"
        case JIGObject(attrs) => {for ((k:String, v:JIGValue) <- attrs) yield "\"" + k + "\" = " + v.toJSON(indent + 1)}.mkString(s"$indentstr{$indentstr", s" ,$indentstr", s"$indentstr}$indentstr")
        case JIGArray(values) => {for (v:JIGValue <- values) yield " " + v.toJSON(indent + 1)}.mkString(s"$indentstr($indentstr", s" ,$indentstr", s"$indentstr)$indentstr")
      }
    }
  }
